...
def constructBayesNet(gameState: hunters.GameState):
    ...
    observation_domain = list(range(0, max_possible_distance + MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factors_to_combine = [f for f in current_factors if var in f.variablesSet()]
            if factors_to_combine:
                combined_factor = joinFactorsByVariable(factors_to_combine, var)[1]

                if len(combined_factor.unconditionedVariables()) > 1:
                    new_factor = eliminate(combined_factor, var)
                    current_factors = [f for f in current_factors if f not in factors_to_combine] + [new_factor]
                else:
                    current_factors = [f for f in current_factors if f not in factors_to_combine]

        final_factor = joinFactors(current_factors)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, value in self.items():
            total += value
            cumulative_prob.append((total, key))

        rand = random.uniform(0, total)

        for prob, key in cumulative_prob:
            if rand < prob:
                return key
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            newBeliefs[ghostPosition] = self.beliefs[ghostPosition] * likelihood
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        extra_particles = self.numParticles % num_positions
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            ...
            newBeliefs[particle] += likelihood
