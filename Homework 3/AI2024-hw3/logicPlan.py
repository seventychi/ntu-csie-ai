...
def sentence3() -> Expr:
    ...
    return conjoin([PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0)), ~(PacmanAlive_0 & PacmanBorn_0),
                    PacmanBorn_0])
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model_pacman_at_x1_y1 = findModel(conjoin(KB + [pacman_x1_y1]))
    model_pacman_not_at_x1_y1 = findModel(conjoin(KB + [~pacman_x1_y1]))

    return model_pacman_at_x1_y1, model_pacman_not_at_x1_y1
...
def foodLogicPlan(problem) -> List:
    ...
    while True:
        ...
        KB.extend([pacmanSuccessorAxiomSingle(x, y, time=t + 1, walls_grid=walls) for (x, y) in non_wall_coords])

        KB.extend(
            [(PropSymbolExpr(pacman_str, x, y, time=t) |
              ~PropSymbolExpr(food_str, x, y, time=t)) %
             ~PropSymbolExpr(food_str, x, y, time=t + 1) for
             (x, y) in all_coords])
