...
def train(agent, env):
    ...
    # plt.xlabel('Step')
    # plt.ylabel('AvgScore')
    # plt.title('AvgScore over training')
    #
...
def main():
    ...
    for dir in os.listdir(save_dir):
        if dir.endswith('.pth'):
            ...
            score_1 = evaluate(agent=None, eval_env=eval_env, capture_frames=False)

            if score_1 > score:
                score = score_1
                best_model_path = file
    # copy the best model to the submission folder
    shutil.copyfile(best_model_path, "./submissions/pacman_dqn.pt")
